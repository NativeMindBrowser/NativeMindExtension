...
export const chatWithEnvironment = definePrompt(async (question: string, contextAttachmentStorage: ContextAttachmentStorage) => {
  ...
  if (currentTab) {
    const { value: { tabId, title = '' } } = currentTab
    tabContextBuilder.insertContent(`- Tab ID ${tabId} (SELECTED): "${title}"`)
  }
  ...
  for (let i = 0; i < imagesMeta.length; i++) {
    ...
    imageContextBuilder.insertContent(`- Image ID ${i + 1}: ${img.value.name}`)
  }
...
${environmentTagBuilder}`.trim()
  ...
  return { user: UserPrompt.fromText(user), system }
})
