...
<view_pdf>
  <pdf_id>this is not a number</pdf_id>
</view_pdf>

<search_online>
<query>example query</query>
missing max_results tag here
</search_online>
`
    ...
    expect(errors.join('')).toContain('Failed to parse parameter <pdf_id>')
    expect(errors.join('')).toContain('Missing required parameter <max_results>')
  })

  it('should parse the tag', async () => {
    const tagWalker = new TagWalker(['view_tab'])
    const text = [
      'start',
      '<view_tab><tab_id>',
      '1</tab_id>',
...
`
  ...
  it('should parse the multiple different tags', async () => {
    const tagWalker = new TagWalker(['view_tab', 'view_image'])
    const input = `start
<view_tab><tab_id>1</tab_id></view_tab>
